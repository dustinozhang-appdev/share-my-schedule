class UserSessionsController < ApplicationController
  ...
  def create_cookie
    ...
    if user != nil
      ...
      if are_they_legit == false
        redirect_to("/user_sign_in?wrongpassword=1", { :alert => "Incorrect password." })
      else
        session.store(:user_id, user.id)
      
        redirect_to("/?justsignedin=1", { :notice => "Signed in successfully." })
      end
    else
      redirect_to("/user_sign_in?nouser=1", { :alert => "No user with that email address." })
    end
  end
  ...
  def destroy_cookies
    ...
    redirect_to("/?justsignedout=1", { :notice => "Signed out successfully." })
  end
